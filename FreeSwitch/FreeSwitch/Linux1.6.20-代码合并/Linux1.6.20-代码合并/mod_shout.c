...
static inline void free_context(shout_context_t *context)
{
	...
	if (context) {
		...
		if (context->fp>0 && context->lame_ready) {//yhy2018-04-18
			...
			if (len) {
				ret = write(context->fp,mp3buffer,len); //yhy fwrite(mp3buffer, 1, len, context->fp);
			}
			...
			lame_mp3_tags_fid(context->gfp, NULL);//yhy context->fp);
		}

		if (context->fp>0) {//yhy2018-04-18
			close(context->fp);
			context->fp = 0;//yhy2018-04-18
		}
	...
	if (switch_test_flag(handle, SWITCH_FILE_FLAG_READ)) {
		...
		if (handle->handler) {
			...
			*host++ = '\0';
			...
			if ((context->fp = open(path, O_WRONLY|O_CREAT|O_APPEND|O_BINARY,0755))<=0) {//yhy2018-04-18
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Error opening %s\n", path);
				goto error;
			}
		}
...
static switch_status_t shout_file_write(switch_file_handle_t *handle, void *data, size_t *len)
{
	...
	if (rlen) {
		//yhy int ret = (int)fwrite(context->mp3buf, 1, rlen, context->fp);
		int ret = (int)write(context->fp,context->mp3buf,rlen);
		if (ret < 0) {
			return SWITCH_STATUS_FALSE;
		}
	}
