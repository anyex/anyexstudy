...
static switch_status_t audio_bridge_on_exchange_media(switch_core_session_t *session)
{
		...
		if (switch_channel_test_flag(channel, CF_INTERCEPTED)) {
			...
			if (switch_channel_test_flag(channel, CF_INTERCEPT)) {
				...
				if (!switch_channel_test_flag(channel, CF_ANSWERED)) {
					...
					//yhy2016-03-28 uuid_bridge 之后会挂机。改为判断一下兼容1.2.23 的fsg.cfg(实际上不设置也可以,通过设置park_after_bridge=true也可以) 是否需要挂机。配合 <action application="export" data="hangup_after_bridge=true"/>使用
					if (switch_true(switch_channel_get_variable(channel, SWITCH_HANGUP_AFTER_BRIDGE_VARIABLE)))
					{
						switch_channel_hangup(channel, SWITCH_CAUSE_NORMAL_CLEARING);
					}
					else
						switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_NOTICE, "yhy park after uuid_bridge.\n");
				}
			}
		}
	}
	...
	if (switch_channel_get_state(channel) == CS_EXCHANGE_MEDIA) {
		...
		switch_channel_set_variable(channel, "park_timeout", "300");//yhy2016-03-28 uuid_bridge 之后会挂机。改为park 300s
