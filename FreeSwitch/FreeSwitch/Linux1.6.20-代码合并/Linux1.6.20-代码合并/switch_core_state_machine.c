...
static void switch_core_standard_on_execute(switch_core_session_t *session)
{
  ...
  top:
	...
	if (switch_channel_ready(session->channel) && switch_channel_get_state(session->channel) == CS_EXECUTE) {
		//yhy2014-01-03  
		if (switch_channel_test_flag(session->channel, CF_ANSWERED))//yhy2014-04-19 
		{
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING, "%s last dialplan,yhy park after uuid_bridge!\n", switch_channel_get_name(session->channel));
			switch_channel_set_variable(session->channel, "park_timeout", "72000");
			switch_channel_set_state(session->channel, CS_PARK);
		}
		else
		{
